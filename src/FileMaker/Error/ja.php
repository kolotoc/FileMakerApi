...
/**
 * src error codes - Japanese translations.
 *
 * @package src
 *
...
$__FM_ERRORS = array(
    ...
    403 => '評価版の src Pro に設定されている最大レコード数の制限を超過しています',
    404 => 'ソート優先順位が無効です',
    ...
    409 => 'インポート順が無効です',
    410 => 'エクスポート順が無効です',
    412 => 'ファイルの修復に、誤ったバージョンの src Pro が使用されました',
    ...
    806 => 'このバージョンの src Pro ではファイルを開くことができません',
    807 => 'ファイルが src Pro のファイルではないか、重大な損傷があります',
    808 => 'アクセス権情報が壊れているため、ファイルを開くことができません',
    809 => 'ディスク/ボリュームがいっぱいです',
    810 => 'ディスク/ボリュームがロックされています',
    811 => 'テンポラリファイルを src Pro ファイルとして開くことができません',
    ...
    1222 => '計算式を保存できません',
    1223 => '指定された関数は存在しません',
    1400 => 'ODBC クライアントドライバの初期化に失敗しました。ODBC クライアントドライバが適切にインストールされていることを確認してください。 注意   ODBC 経由でデータを共有するプラグインコンポーネントは、src Server によって自動的にインストールされます。ODBC クライアントドライバは、src Server Web Publishing CD を使用してインストールされます。詳細については、『src ODBC および JDBC クライアントドライバのインストール』を参照してください。',
