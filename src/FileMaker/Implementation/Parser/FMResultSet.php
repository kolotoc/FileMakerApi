...
class FileMaker_Parser_FMResultSet
{
    ...
    function parse($V0f635d0e)
    {
        ...
        if (version_compare($this->Vf5bf48aa['version'], FileMaker::getMinServerVersion(), '<')) {
            return new FileMaker_Error($this->_fm, 'This API requires at least version ' . FileMaker::getMinServerVersion() . ' of src Server to run (detected ' . $this->Vf5bf48aa['version'] . ').');
        }
        $this->V6de51026 = true;
        return true;
    }

    function setResult($Vb4a88417)
    {
        if (!$this->V6de51026) {
        ...
        foreach ($this->V6e52c40b as $Vde17f0f2) {
            $V4b43b0ae = new FileMaker_Record($Vb4a88417->_impl->_layout);
            ...
            if ($Vde17f0f2['children']) {
                foreach ($Vde17f0f2['children'] as $Vaca007a7 => $V268184c1) {
                    ...
                    foreach ($V268184c1 as $V1b7d5726) {
                        $V4a8a08f0 = new FileMaker_Record($Vb4a88417->_impl->_layout->getRelatedSet($Vaca007a7));
